...
| ECC 부속 규칙 | `.claude/rules/ecc/` | common·java·python·typescript·web 패턴·테스트 (java/python/ts/web 은 `paths:` 스코프 — 해당 파일 작업 시에만 주입) |
| Claude 설정 | `.claude/settings.json` | caveman 훅 + `permissions.ask` (rm -rf·force push·reset --hard·clean -fd·branch -D 실행 전 확인) |
| 규칙 | `docs/rules/` | 7개 파일 (01~07) |
| 빠른 시작 | `QUICKSTART.md` | 기존 파일 존재 시 스킵 |
| gitignore 기본값 | `.gitignore` | 기존 파일 존재 시 스킵 (다언어 기본) |
...
|---|---|
| `Agent(subagent_type="<name>", description=X, prompt="[역할] .../agents/<name>.md ...")` | `task(subagent_type="<name>", load_skills=[...], description=X, prompt=...)` — prompt 의 `agents/<name>.md` 에서 실제 agent 이름을 재추출해 `subagent_type` 에 넣고, planner/implementer 는 `load_skills`(speckit-*) 자동 주입 |
