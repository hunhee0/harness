...
>
> **opencode 환경**: `setup.ps1 -Opencode` / `setup.sh --opencode` 모드가 빌드 시 자동 변환 — subagent 호출(Agent 도구 → task 도구), 사용자 질문 도구(→ STOP 텍스트 응답 대기), 디렉터리 경로(claude 계열 → opencode 계열). 원본은 Claude Code 형식으로 유지.
