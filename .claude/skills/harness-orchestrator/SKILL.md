...
> **opencode/devai 환경**: `setup -Opencode` 변환이 에이전트 호출 구문을 `task(...)` 로,
> 디렉터리 prefix 를 `.opencode/` 로, 사용자 확인 도구 호출을 STOP 텍스트로 자동 치환한다.
> 원본은 Claude Code 형식 (`Agent()` 호출) 으로 유지.
>
> **자연어 트리거** ("기능 만들어줘", "구현해줘", "이어서 진행", "수정", "보완" 등) 수신 시
> 이 스킬이 Phase 0 → 5 파이프라인을 직접 운영한다: **planner → implementer → reviewer → qa**.
> 각 Phase 에서 해당 agent 를 `Agent()` (opencode 변환 후 `task()`) 로 호출한다.
>
...
> 게이트는 오케스트레이터 메인 컨텍스트가 `AskUserQuestion` 으로 제시한다 (subagent 안에 두지 말 것).
...
**constitution 점검 (1회)**: `.specify/memory/constitution.md` 가 없거나 플레이스홀더 상태(`[PROJECT_NAME]`·`[PRINCIPLE_` 문자열 포함)면 Phase 1 진입 전에 경고하고 `/speckit-constitution` 작성을 권장한다 (하드 블록 아님 — 사용자가 건너뛰기 선택 가능).
...
```
# 같은 응답 안에서 N개 Agent 도구를 병렬 호출 (subagent_type 에 실제 agent 이름)
Agent(subagent_type="architect", description="...", prompt="[역할] .claude/agents/architect.md ...")
Agent(subagent_type="architect", description="...", prompt="[역할] .claude/agents/architect.md ...")
Agent(subagent_type="code-architect", description="...", prompt="[역할] .claude/agents/code-architect.md ...")
```
...
```
# 예시는 planner — 호출 대상의 실제 agent 이름으로 치환할 것 (subagent_type + prompt 첫 줄 모두)
Agent(
  subagent_type="planner",
  description="<3~5단어 작업 요약>",
  prompt="""
  [역할] .claude/agents/planner.md 를 먼저 읽고 그 정의대로 행동할 것.
...
- **subagent_type**: 원본에도 **실제 agent 이름**(planner·implementer·reviewer·qa 등)을 직접 기재한다 — Claude Code 가 해당 agent frontmatter 의 `tools`·`model` 을 그대로 적용한다. 변환기는 subagent_type 값과 무관하게 prompt 첫 줄의 `.../agents/<name>.md` 에서 이름을 재추출하므로(devai 도 실제 등록 이름 요구), **prompt 첫 줄에 `agents/<정확한 이름>.md` 를 반드시 명시**한다. `general-purpose` 는 이름 추출 실패 시 변환기 폴백일 뿐 — 원본에 쓰지 말 것.
...
- `model` 파라미터 미지정 — agent frontmatter 의 `model` 이 적용됨 (Claude Code: `sonnet` 등 / opencode 변환 후: `ABCLab/[KTDS] Qwen...`)
...
각 게이트는 `AskUserQuestion` 으로 "승인 / 수정 / 중단" 제시. **승인 없이 다음 단계 진입 금지.**
...
```
Agent(subagent_type="python-reviewer", description="python review", prompt="[역할] .claude/agents/python-reviewer.md ...")
Agent(subagent_type="security-reviewer", description="security review", prompt="[역할] .claude/agents/security-reviewer.md ...")
Agent(subagent_type="code-reviewer", description="general review", prompt="[역할] .claude/agents/code-reviewer.md ...")
```
