...
> **호출 경로**: harness-orchestrator Phase 3 에서 reviewer 서브에이전트로 호출됨 (호출 형식 — Claude Code `Agent()` / opencode 변환 후 `task()` — 은 orchestrator SKILL.md 참조). Phase 2 (implementer) 완료 후 자동 진입.
> - 1차 스펙 미달 → Phase 2 재호출
> - 2차 팬아웃 결과 CRITICAL/HIGH ≥ 1 → Phase 2 재호출
> - 모두 MEDIUM/LOW → Phase 4 (qa) 진입
